...
public class GameAdapter extends RecyclerBaseAdapter<Game> {

    public GameAdapter(List<Game> datas) {
        super(datas);
    }
...
//                tvWinner.setText(getWinner(data.getFinish()));
    ...
    private String getWords(Integer id) {
        if(null == id)
            return "";
        Word word = new Word();// = DbUtils.getWordService().findById(id);
        if(null == word)
            return "";
        return word.getW1() + "\n" + word.getW2();
    }
