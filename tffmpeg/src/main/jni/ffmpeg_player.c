...
JNIEXPORT void JNICALL Java_com_fg_tffmpeg_VideoUtils_decode(JNIEnv * env,
		jclass jcls, jstring input_jstr, jstring output_jstr) {
	...
	uint8_t *out_buffer = (uint8_t *) av_malloc(
			avpicture_get_size(AV_PIX_FMT_YUV422P, pCodecCtx->width,
					pCodecCtx->height));
	//初始化缓冲区
	avpicture_fill((AVPicture *) pFrameYUV, out_buffer, AV_PIX_FMT_YUV422P,
			pCodecCtx->width, pCodecCtx->height);

	//用于转码（缩放）的参数，转之前的宽高，转之后的宽高，格式等
	LOGI("pix_fmt %d", pCodecCtx->pix_fmt);
	LOGI("AV_PIX_FMT_YUV420P %d", AV_PIX_FMT_YUV422P);
	struct SwsContext *sws_ctx = sws_getContext(pCodecCtx->width,
			pCodecCtx->height, pCodecCtx->pix_fmt, pCodecCtx->width,
			pCodecCtx->height, AV_PIX_FMT_YUV422P, SWS_BICUBIC, NULL, NULL,
			NULL);
	...
	while (av_read_frame(pFormatCtx, packet) >= 0) {
		...
		if (packet->stream_index == v_stream_idx) {
			...
			if (got_picture) {
				...
				LOGE("%s %d", "没有数据帧解析", got_picture);
			}

		} else if (packet->stream_index == a_stream_idx) {
			LOGI("x解码第%d帧 音频帧", frame_count);
		} else {
			LOGI("x解码第%d帧 非音视频帧", frame_count);
		}
