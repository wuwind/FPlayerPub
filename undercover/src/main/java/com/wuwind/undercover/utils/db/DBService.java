...
public class DBService<T> {
    ...
    public long insert(final T t) {
        ...
        abstractDao.getSession().runInTx(new Runnable() {
            @Override
            public void run() {
                id[0] = abstractDao.insertOrReplace(t);
            }
        });
        return id[0];
    }
    ...
    public void deleteAll() {
        instance.abstractDao.deleteAll();
    }
     ...
     * @return
     */
    public List<T> findNotEqList(List<Property> properties, List<Object> values) {
        if (properties.size() != values.size())
            return null;
        QueryBuilder<T> builder = getQueryBuilder();
        for (int i = 0; i < properties.size(); i++) {
            WhereCondition condition = properties.get(i).notEq(values.get(i));
            builder = builder.where(condition);
        }
        return builder.list();
    }

    public List<T> findNotEqList(Property property, Object value) {
        QueryBuilder<T> builder = getQueryBuilder();
